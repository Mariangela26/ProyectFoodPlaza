...
public class RestEmployeeRestController {
    private  final IRestEmployeeHandler restEmployeeHandler;
    ...
    public ResponseEntity<Void> saveRestaurantEmployee(@Valid @RequestBody RestEmployeeRequestDto restEmployeeRequestDto) {
        restEmployeeHandler.saveRestEmployee(restEmployeeRequestDto);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "All restaurant_employees returned",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = RestEmployeeResponseDto.class)))),
            @ApiResponse(responseCode = "404", description = "No data found", content = @Content)
    })
    @GetMapping("/")
    @PreAuthorize("hasAuthority('PROPIETARIO')")
    public ResponseEntity<List<RestEmployeeResponseDto>> getAllRestaurantEmployees() {
        return ResponseEntity.ok(restEmployeeHandler.getAllRestEmployees());
    }
